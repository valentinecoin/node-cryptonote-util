...
#include "hash-ops.h"
  ...
  */
  ...
  private:
    ...
    void *data;
    friend inline void cn_slow_hash(cn_context &, const void *, size_t, Hash &, int variant);
  };

  inline void cn_slow_hash(cn_context &context, const void *data, size_t length, Hash &hash, int variant = 0) {
    (*cn_slow_hash_f)(context.data, data, length, reinterpret_cast<void *>(&hash),variant,0);
  }
